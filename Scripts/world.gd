...
func add_player(peer_id):
	...
	apply_team(peer_id, 1 if smallest_team == "Team1" else 2) # host must skip straight to apply_team since they need to add the peer to their group so others can replicate
	upd_teams.rpc(get_tree().get_nodes_in_group("Team1").map(func(node): return node.name), get_tree().get_nodes_in_group("Team2").map(func(node): return node.name)) # converts nodes in each team to peer ids before sending rpc
...
@rpc("call_local")
func upd_teams(team1, team2): # reload teams for everyone upon new peer connecting, ignore peers who are already loaded
...
func apply_team(plr_id, team): # keeps upd_teams relatively dry and resolves host group conflict
	...
	player_node.add_to_group("Team%s" % team)

...
func on_window_mode_selected(index : int):
	match index:
		0: #Fullscreen
			...
			Save.game_data["Resolution"] = "Fullscreen"
		1: #Window Mode
			DisplayServer.window_set_mode(DisplayServer.WINDOW_MODE_WINDOWED)
			DisplayServer.window_set_flag(DisplayServer.WINDOW_FLAG_BORDERLESS,false)
			get_window().set_size(Save.game_data["Resolution"] as Vector2i)
			Save.game_data["Resolution"] = "Windowed"
		2: #Borderless Window
			DisplayServer.window_set_mode(DisplayServer.WINDOW_MODE_WINDOWED)
			DisplayServer.window_set_flag(DisplayServer.WINDOW_FLAG_BORDERLESS,true)
			get_window().set_size(Save.game_data["Resolution"] as Vector2i)
			Save.game_data["Resolution"] = "Borderless Window"
		3: #Borderless Fullscreen
			DisplayServer.window_set_mode(DisplayServer.WINDOW_MODE_FULLSCREEN)
			DisplayServer.window_set_flag(DisplayServer.WINDOW_FLAG_BORDERLESS,true)
			Save.game_data["Resolution"] = "Borderless Fullscreen"
	Save.save_data()
